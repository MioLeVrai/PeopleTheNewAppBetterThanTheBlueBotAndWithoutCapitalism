(() => {
  ...
  function ownMessage(unit) {
    if (scopeFor(unit) === "dm") {
      return Boolean(unit.closest(".dm-message.mine"));
    }

    const row = unit.closest(".message");
    const author = String(row?.querySelector(".message-head strong")?.textContent || "").trim().toLocaleLowerCase("fr-FR");
    const me = String(document.getElementById("profileName")?.textContent || "").trim().toLocaleLowerCase("fr-FR");
    return Boolean(author && me && author === me);
  }

  async function deleteMessage(unit) {
    const id = String(unit?.dataset?.messageId || "");
    if (!id) return;

    if (scopeFor(unit) === "dm") {
      const response = await fetch("/api/dm/message/" + encodeURIComponent(id), { method: "DELETE" });
      const data = await response.json().catch(() => ({}));
    ...
    if (!peopleSocket?.emit) throw new Error("Connexion au chat indisponible.");

    await new Promise((resolve, reject) => {
      peopleSocket.emit("chat-message-delete", { id }, (response) => {
        if (!response?.ok) reject(new Error(response?.error || "Suppression impossible."));
        else resolve();
      });
    });
  }
  ...
  document.addEventListener("contextmenu", (event) => {
    ...
    queueMicrotask(() => {
      ...
      if (ownMessage(unit) && !menu.querySelector(".people-message-menu-item.delete")) {
        const remove = document.createElement("button");
        remove.type = "button";
        remove.className = "people-message-menu-item delete";
        remove.textContent = "🗑 Supprimer";
        remove.addEventListener("click", async () => {
          menu.remove();
          try { await deleteMessage(unit); }
