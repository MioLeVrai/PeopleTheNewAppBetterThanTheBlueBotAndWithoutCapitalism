...
// === PEOPLE_MESSAGE_IMAGES_V2_START ===
/*
  Pièces jointes People.
  - Salons serveur : fichier chiffré au repos en AES-256-GCM.
  - MP : le navigateur envoie un conteneur E2EE opaque, lui-même chiffré au repos.
  - Compatibilité : les anciennes images et anciens conteneurs E2EE v1 restent lisibles.
*/
const PEOPLE_MESSAGE_IMAGE_MAX_MB = Math.max(
  1,
  Math.min(
    100,
    Number(process.env.PEOPLE_ATTACHMENT_MAX_MB) || 25
  )
);
const PEOPLE_MESSAGE_IMAGE_MAX_BYTES = Math.floor(
  PEOPLE_MESSAGE_IMAGE_MAX_MB * 1024 * 1024
);
const PEOPLE_MESSAGE_IMAGE_TRANSPORT_MAX_BYTES =
  PEOPLE_MESSAGE_IMAGE_MAX_BYTES + 1024 * 1024;

const PEOPLE_DM_E2EE_IMAGE_MIME =
  "application/x-people-e2ee-image";
const PEOPLE_DM_E2EE_IMAGE_MAGIC = Buffer.from(
  "PEOPLE-E2EE-IMAGE-V1\n",
  "ascii"
);
const PEOPLE_MESSAGE_ATTACHMENT_MAGIC = Buffer.from(
  "PEOPLE-ATTACHMENT-V1\n",
  "ascii"
);
const PEOPLE_MESSAGE_IMAGE_STORAGE_MAGIC = Buffer.from(
  "PEOPLE-IMG-STORE-V1\0",
  "ascii"
);
const PEOPLE_MESSAGE_IMAGE_STORAGE_AAD = Buffer.from(
  "People message image storage v1",
  "utf8"
);
const PEOPLE_INLINE_IMAGE_MIMES = new Set([
  "image/jpeg",
  "image/png",
  "image/webp",
  "image/gif"
]);
const PEOPLE_INLINE_VIDEO_MIMES = new Set([
  "video/mp4",
  "video/webm",
  "video/ogg",
  "video/quicktime"
...
function peopleAttachmentMime(value) {
...
function peopleAttachmentName(value, fallback = "fichier") {
...
function peopleAttachmentNameFromHeader(req) {
  const raw = String(req.headers["x-people-file-name"] || "");
  if (!raw) return "fichier";
  try {
    return peopleAttachmentName(decodeURIComponent(raw));
  } catch {
    return peopleAttachmentName(raw);
  }
}

function peoplePackMessageAttachment(buffer, name, mime) {
  const body = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer || []);
  const meta = Buffer.from(
    JSON.stringify({
      v: 1,
      name: peopleAttachmentName(name),
      mime: peopleAttachmentMime(mime),
      size: body.length
    }),
    "utf8"
  );
  if (meta.length > 4096) throw new Error("ATTACHMENT_META_INVALID");
  const length = Buffer.allocUnsafe(4);
  length.writeUInt32BE(meta.length, 0);
  return Buffer.concat([
    PEOPLE_MESSAGE_ATTACHMENT_MAGIC,
    length,
    meta,
    body
  ]);
...
function peopleUnpackMessageAttachment(buffer, fallbackMime = "application/octet-stream") {
  if (!Buffer.isBuffer(buffer)) return null;
  const magic = PEOPLE_MESSAGE_ATTACHMENT_MAGIC;
  if (
    buffer.length < magic.length + 4 ||
    !buffer.subarray(0, magic.length).equals(magic)
  ) {
    return {
      data: buffer,
      mime: peopleAttachmentMime(fallbackMime),
      name: peopleAttachmentName(
        PEOPLE_INLINE_IMAGE_MIMES.has(peopleAttachmentMime(fallbackMime))
          ? "image"
          : "fichier"
      ),
      size: buffer.length,
  ...
  try {
    ...
    if (metaLength < 16 || metaLength > 4096) throw new Error("ATTACHMENT_META_INVALID");
    const metaStart = magic.length + 4;
    const metaEnd = metaStart + metaLength;
    if (metaEnd > buffer.length) throw new Error("ATTACHMENT_META_INVALID");
    const meta = JSON.parse(buffer.subarray(metaStart, metaEnd).toString("utf8"));
    const data = buffer.subarray(metaEnd);
    const size = Number(meta?.size);
    if (meta?.v !== 1 || !Number.isInteger(size) || size !== data.length) {
      throw new Error("ATTACHMENT_META_INVALID");
    }
    return {
      data,
      mime: peopleAttachmentMime(meta?.mime),
      name: peopleAttachmentName(meta?.name),
      size,
      legacy: false
    };
  } catch (cause) {
    const err = new Error("ATTACHMENT_META_INVALID");
    err.code = "ATTACHMENT_META_INVALID";
    err.cause = cause;
    throw err;
  }
}

function peopleMessageImageStorageIsEncrypted(buffer) {
  return Buffer.isBuffer(buffer) &&
    buffer.length >= PEOPLE_MESSAGE_IMAGE_STORAGE_MAGIC.length &&
    buffer.subarray(0, PEOPLE_MESSAGE_IMAGE_STORAGE_MAGIC.length)
      .equals(PEOPLE_MESSAGE_IMAGE_STORAGE_MAGIC);
}

function peopleEncryptMessageImageData(buffer) {
  if (!Buffer.isBuffer(buffer)) throw new TypeError("Le contenu doit être un Buffer.");
  if (peopleMessageImageStorageIsEncrypted(buffer)) return buffer;

  const iv = cryptoAccounts.randomBytes(12);
  const cipher = cryptoAccounts.createCipheriv(
    "aes-256-gcm",
    PEOPLE_MESSAGE_ENCRYPTION_KEY,
    iv
  );
  cipher.setAAD(PEOPLE_MESSAGE_IMAGE_STORAGE_AAD);
  const ciphertext = Buffer.concat([cipher.update(buffer), cipher.final()]);
  const tag = cipher.getAuthTag();
  return Buffer.concat([
    PEOPLE_MESSAGE_IMAGE_STORAGE_MAGIC,
    iv,
    tag,
    ciphertext
  ]);
...
function peopleDecryptMessageImageData(buffer) {
  if (!Buffer.isBuffer(buffer)) return Buffer.alloc(0);
  if (!peopleMessageImageStorageIsEncrypted(buffer)) return buffer;

  const offset = PEOPLE_MESSAGE_IMAGE_STORAGE_MAGIC.length;
  if (buffer.length < offset + 28) throw new Error("Pièce jointe chiffrée au repos invalide.");
  const iv = buffer.subarray(offset, offset + 12);
  const tag = buffer.subarray(offset + 12, offset + 28);
  const ciphertext = buffer.subarray(offset + 28);
  const decipher = cryptoAccounts.createDecipheriv(
    "aes-256-gcm",
    PEOPLE_MESSAGE_ENCRYPTION_KEY,
    iv
  );
  decipher.setAAD(PEOPLE_MESSAGE_IMAGE_STORAGE_AAD);
  decipher.setAuthTag(tag);
  return Buffer.concat([decipher.update(ciphertext), decipher.final()]);
}

function peopleDmE2eeImageContainer(buffer) {
  if (
    !Buffer.isBuffer(buffer) ||
    buffer.length < PEOPLE_DM_E2EE_IMAGE_MAGIC.length + 4 + 32 ||
    !buffer.subarray(0, PEOPLE_DM_E2EE_IMAGE_MAGIC.length).equals(PEOPLE_DM_E2EE_IMAGE_MAGIC)
  ) {
    return null;
  }

  try {
    const headerLength = buffer.readUInt32BE(PEOPLE_DM_E2EE_IMAGE_MAGIC.length);
    if (headerLength < 32 || headerLength > 64 * 1024) throw new Error("E2EE_IMAGE_HEADER_INVALID");
    const headerStart = PEOPLE_DM_E2EE_IMAGE_MAGIC.length + 4;
    const headerEnd = headerStart + headerLength;
    if (headerEnd >= buffer.length) throw new Error("E2EE_IMAGE_HEADER_INVALID");

    const envelope = JSON.parse(buffer.subarray(headerStart, headerEnd).toString("utf8"));
    const version = Number(envelope?.v);
    const from = String(envelope?.from || "");
    const to = String(envelope?.to || "");
    const senderDevice = peopleE2eeDeviceId(envelope?.sd);
    const senderPublic = peopleE2eePublicJwk(envelope?.spk);
    const iv = String(envelope?.iv || "");
    const mime = peopleAttachmentMime(envelope?.mime);
    const name = peopleAttachmentName(
      envelope?.name,
      version === 1 ? "image" : "fichier"
    );
    const size = Number(envelope?.size);
    const keys = Array.isArray(envelope?.keys) ? envelope.keys : [];
    const v1MimeOk = version !== 1 || PEOPLE_INLINE_IMAGE_MIMES.has(mime);
    const v2NameOk = version !== 2 || String(envelope?.name || "").trim().length > 0;

    if (
      ![1, 2].includes(version) ||
      !from || !to || from.length > 100 || to.length > 100 ||
      !senderDevice || !senderPublic ||
      !/^[a-zA-Z0-9_-]{16,40}$/.test(iv) ||
      mime === PEOPLE_DM_E2EE_IMAGE_MIME || !v1MimeOk || !v2NameOk ||
      !Number.isInteger(size) || size < 1 || size > PEOPLE_MESSAGE_IMAGE_MAX_BYTES ||
      keys.length < 1 || keys.length > 16
    ) {
      throw new Error("E2EE_IMAGE_HEADER_INVALID");
    }

    const normalizedKeys = keys.map((item) => {
      const userId = String(item?.u || "");
      const deviceId = peopleE2eeDeviceId(item?.d);
      const wrapIv = String(item?.iv || "");
      const wrapped = String(item?.ct || "");
      if (
        !userId || userId.length > 100 || !deviceId ||
        !/^[a-zA-Z0-9_-]{16,40}$/.test(wrapIv) ||
        !/^[a-zA-Z0-9_-]{32,160}$/.test(wrapped)
      ) {
        throw new Error("E2EE_IMAGE_HEADER_INVALID");
      }
      return { u: userId, d: deviceId, iv: wrapIv, ct: wrapped };
    });

    const ciphertext = buffer.subarray(headerEnd);
    if (ciphertext.length !== size + 16) throw new Error("E2EE_IMAGE_CIPHERTEXT_INVALID");
    return {
      v: version,
      from,
      to,
    ...
    const err = new Error("E2EE_IMAGE_INVALID");
    err.code = "E2EE_IMAGE_INVALID";
    err.cause = cause;
    throw err;
  }
}

function peopleDmE2eeImageMatchesConversation(envelope, senderId, recipientId) {
  if (!envelope) return false;
  const sender = String(senderId);
  const recipient = String(recipientId);
  const allowedIds = new Set([sender, recipient]);
  const keys = Array.isArray(envelope.keys) ? envelope.keys : [];
  return (
    envelope.from === sender &&
    envelope.to === recipient &&
    keys.some((item) => String(item?.u) === sender) &&
    keys.some((item) => String(item?.u) === recipient) &&
    !keys.some((item) => !allowedIds.has(String(item?.u)))
  );
}

const PEOPLE_LOCAL_MESSAGE_IMAGE_DIR = pathAccounts.join(__dirname, "people-message-images");
const PEOPLE_LOCAL_MESSAGE_IMAGE_META = pathAccounts.join(__dirname, "people-message-images.local.json");
const peopleMessageImageRate = new Map();

function peopleMessageImageMime(buffer) {
  if (!Buffer.isBuffer(buffer)) return null;
  if (
    buffer.length >= 8 &&
...
function peopleMessageImageExtension(mime) {
  return {
    "image/jpeg": ".jpg",
    "image/png": ".png",
    "image/webp": ".webp",
    ...
    "video/ogg": ".ogv",
    "video/quicktime": ".mov",
    [PEOPLE_DM_E2EE_IMAGE_MIME]: ".bin"
  }[mime] || ".bin";
}

function peopleNormalizeMessageImageId(value) {
  return String(value || "").trim().slice(0, 100);
}

function peopleMessageImageUploadAllowed(accountId) {
  const key = String(accountId);
  const now = Date.now();
  const recent = (peopleMessageImageRate.get(key) || []).filter((time) => now - time < 60 * 1000);
  if (recent.length >= 12) {
    peopleMessageImageRate.set(key, recent);
    return false;
  }
  recent.push(now);
  peopleMessageImageRate.set(key, recent);
...
function peopleReadLocalMessageImageMeta() {
  try {
    if (!fsAccounts.existsSync(PEOPLE_LOCAL_MESSAGE_IMAGE_META)) return {};
    const raw = JSON.parse(fsAccounts.readFileSync(PEOPLE_LOCAL_MESSAGE_IMAGE_META, "utf8"));
    return raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
  } catch { return {}; }
}

function peopleWriteLocalMessageImageMeta(meta) {
  fsAccounts.writeFileSync(
...
const PEOPLE_MESSAGE_IMAGE_CLEANUP_INTERVAL_MS = 60 * 60 * 1000;

async function peopleCreatePendingMessageImage(ownerId, buffer, uploadMeta = {}) {
  if (
    !Buffer.isBuffer(buffer) || !buffer.length ||
    buffer.length > PEOPLE_MESSAGE_IMAGE_TRANSPORT_MAX_BYTES
  ) {
    const err = new Error("IMAGE_SIZE");
    err.code = "IMAGE_SIZE";
    throw err;
  }

  const owner = String(ownerId);
  let e2eeImage = peopleDmE2eeImageContainer(buffer);
  let mime;
  let plainForStorage;
  let name;

  if (e2eeImage) {
    if (e2eeImage.from !== owner) {
      const err = new Error("E2EE_IMAGE_INVALID");
      err.code = "E2EE_IMAGE_INVALID";
      throw err;
    }
    mime = PEOPLE_DM_E2EE_IMAGE_MIME;
    name = "piece-jointe-e2ee.bin";
    plainForStorage = buffer;
  } else {
    if (buffer.length > PEOPLE_MESSAGE_IMAGE_MAX_BYTES) {
      const err = new Error("IMAGE_SIZE");
      err.code = "IMAGE_SIZE";
      throw err;
    }
    const declaredMime = peopleAttachmentMime(uploadMeta.mime);
    const sniffedImage = peopleMessageImageMime(buffer);
    mime = sniffedImage || declaredMime;
    name = peopleAttachmentName(uploadMeta.name, sniffedImage ? "image" : "fichier");
    plainForStorage = peoplePackMessageAttachment(buffer, name, mime);
  }

  const storedData = peopleEncryptMessageImageData(plainForStorage);
  const storageMime = mime.length <= 32 ? mime : "application/octet-stream";

  if (peoplePool) {
    const now = Date.now();
    if (now - peopleMessageImageLastCleanupAt >= PEOPLE_MESSAGE_IMAGE_CLEANUP_INTERVAL_MS) {
      peopleMessageImageLastCleanupAt = now;
      try {
        await peoplePool.query(
        ...
        console.warn("[People attachment cleanup]", err?.message || err);
      }
    }

    const result = await peoplePool.query(
      "INSERT INTO people_message_images (owner_id, mime_type, data) VALUES ($1, $2, $3) RETURNING id",
      [owner, storageMime, storedData]
    );
    return { id: String(result.rows[0].id), mime };
  }

  fsAccounts.mkdirSync(PEOPLE_LOCAL_MESSAGE_IMAGE_DIR, { recursive: true });
  const id = cryptoAccounts.randomUUID();
  const file = id + peopleMessageImageExtension(mime);
  fsAccounts.writeFileSync(pathAccounts.join(PEOPLE_LOCAL_MESSAGE_IMAGE_DIR, file), storedData);

  const meta = peopleReadLocalMessageImageMeta();
  meta[id] = {
    id,
    ownerId: owner,
    mime: storageMime,
    file,
    e2eeFrom: e2eeImage?.from || null,
    e2eeTo: e2eeImage?.to || null,
    scope: null,
    messageId: null,
    senderId: null,
    recipientId: null,
  ...
  peopleWriteLocalMessageImageMeta(meta);
  return { id, mime };
}

async function peopleBindGeneralMessageImage(ownerId, imageId, messageId, client = peoplePool) {
  const imageKey = peopleNormalizeMessageImageId(imageId);
  if (!imageKey) return null;
  const owner = String(ownerId);
  const message = String(messageId);

  if (peoplePool) {
    if (!/^\d+$/.test(imageKey)) return null;
    const result = await client.query(
      "UPDATE people_message_images SET general_message_id = $3 " +
      "WHERE id = $1 AND owner_id = $2 AND general_message_id IS NULL " +
      "AND dm_message_id IS NULL AND mime_type <> $4 RETURNING id",
      [imageKey, owner, message, PEOPLE_DM_E2EE_IMAGE_MIME]
    );
    return result.rows[0] ? String(result.rows[0].id) : null;
  }

  const meta = peopleReadLocalMessageImageMeta();
  const item = meta[imageKey];
  if (!item || String(item.ownerId) !== owner || item.scope || item.mime === PEOPLE_DM_E2EE_IMAGE_MIME) return null;
  item.scope = "general";
...
async function peopleBindDmMessageImage(ownerId, imageId, messageId, senderId, recipientId, client = peoplePool) {
  const imageKey = peopleNormalizeMessageImageId(imageId);
  if (!imageKey) return null;
  const owner = String(ownerId);
  const message = String(messageId);
  ...
  if (peoplePool) {
    ...
    const pending = await client.query(
      "SELECT mime_type, data FROM people_message_images " +
      "WHERE id = $1 AND owner_id = $2 AND general_message_id IS NULL " +
      "AND dm_message_id IS NULL FOR UPDATE",
      [imageKey, owner]
    );
    const pendingRow = pending.rows[0];
    if (!pendingRow) return null;
    if (pendingRow.mime_type === PEOPLE_DM_E2EE_IMAGE_MIME) {
      const envelope = peopleDmE2eeImageContainer(
        peopleDecryptMessageImageData(pendingRow.data)
      );
      if (!peopleDmE2eeImageMatchesConversation(envelope, senderId, recipientId)) return null;
    }
    const result = await client.query(
      "UPDATE people_message_images SET dm_message_id = $3 " +
      "WHERE id = $1 AND owner_id = $2 AND general_message_id IS NULL " +
      "AND dm_message_id IS NULL RETURNING id",
      [imageKey, owner, message]
    );
    return result.rows[0] ? String(result.rows[0].id) : null;
  }
  ...
  const meta = peopleReadLocalMessageImageMeta();
  const item = meta[imageKey];
  if (!item || String(item.ownerId) !== owner || item.scope) return null;
  if (item.mime === PEOPLE_DM_E2EE_IMAGE_MIME) {
    const filePath = pathAccounts.join(PEOPLE_LOCAL_MESSAGE_IMAGE_DIR, item.file);
    if (!fsAccounts.existsSync(filePath)) return null;
    const envelope = peopleDmE2eeImageContainer(
      peopleDecryptMessageImageData(fsAccounts.readFileSync(filePath))
    );
    if (!peopleDmE2eeImageMatchesConversation(envelope, senderId, recipientId)) return null;
  }
  item.scope = "dm";
  item.messageId = message;
  item.senderId = String(senderId);
  item.recipientId = String(recipientId);
  meta[imageKey] = item;
  peopleWriteLocalMessageImageMeta(meta);
  return imageKey;
}

function peopleSendMessageAttachment(req, res, attachment) {
  const data = Buffer.isBuffer(attachment?.data) ? attachment.data : Buffer.alloc(0);
  const mime = peopleAttachmentMime(attachment?.mime);
  const name = peopleAttachmentName(attachment?.name);
  const encodedName = encodeURIComponent(name);
  const inline = PEOPLE_INLINE_IMAGE_MIMES.has(mime) || PEOPLE_INLINE_VIDEO_MIMES.has(mime) || mime.startsWith("audio/");

  res.setHeader("Content-Type", mime);
  res.setHeader("X-Content-Type-Options", "nosniff");
  res.setHeader("Cache-Control", "private, max-age=86400");
  res.setHeader("Accept-Ranges", "bytes");
  res.setHeader("X-People-File-Name", encodedName);
  res.setHeader("X-People-Original-Size", String(data.length));
  res.setHeader(
    "Content-Disposition",
  ...
  if (match && data.length) {
    ...
    if (
      ...
      start >= 0 && start < data.length && end >= start
    ) {
...
app.post(
  ...
  express.raw({
    type: () => true,
    limit: PEOPLE_MESSAGE_IMAGE_TRANSPORT_MAX_BYTES
  }),
  async (req, res) => {
    try {
      const session = peopleSessionForRequest(req, res);
      if (!session) return;
      if (!peopleMessageImageUploadAllowed(session.id)) {
        return res.status(429).json({ ok: false, error: "Tu envoies trop de pièces jointes trop vite." });
      }

      const attachment = await peopleCreatePendingMessageImage(
        session.id,
        req.body,
        {
          mime: (() => {
            const raw = String(req.headers["x-people-file-type"] || "");
            if (!raw) return req.headers["content-type"];
            try { return decodeURIComponent(raw); } catch { return raw; }
          })(),
          name: peopleAttachmentNameFromHeader(req)
        }
      );
      res.json({ ok: true, imageId: attachment.id });
    } catch (err) {
      if (err?.type === "entity.too.large" || err?.code === "IMAGE_SIZE") {
        return res.status(413).json({
          ok: false,
          error: `La pièce jointe doit faire moins de ${PEOPLE_MESSAGE_IMAGE_MAX_MB} Mo.`
        });
      }
      if (err?.code === "E2EE_IMAGE_INVALID") {
        return res.status(400).json({ ok: false, error: "Pièce jointe E2EE invalide." });
      }
      console.error("[People chat/attachment upload]", err);
      res.status(500).json({ ok: false, error: "Impossible d'envoyer la pièce jointe." });
    }
  }
);
...
app.get(
  ...
  async (req, res) => {
    try {
      const session = peopleSessionForRequest(req, res);
      if (!session) return;
      const imageId = peopleNormalizeMessageImageId(req.params.id);
      ...
      if (peoplePool) {
        ...
        const raw = peopleDecryptMessageImageData(row.data);
        if (row.mime_type === PEOPLE_DM_E2EE_IMAGE_MIME) {
          return peopleSendMessageAttachment(req, res, {
            data: raw,
            mime: PEOPLE_DM_E2EE_IMAGE_MIME,
            name: "piece-jointe-e2ee.bin"
          });
        }
        const attachment = peopleUnpackMessageAttachment(raw, row.mime_type);
        return peopleSendMessageAttachment(req, res, attachment);
      }

      const meta = peopleReadLocalMessageImageMeta();
      const item = meta[imageId];
      ...
      const filePath = pathAccounts.join(PEOPLE_LOCAL_MESSAGE_IMAGE_DIR, item.file);
      if (!fsAccounts.existsSync(filePath)) return res.status(404).end();
      const raw = peopleDecryptMessageImageData(fsAccounts.readFileSync(filePath));
      if (item.mime === PEOPLE_DM_E2EE_IMAGE_MIME) {
        return peopleSendMessageAttachment(req, res, {
          data: raw,
          mime: PEOPLE_DM_E2EE_IMAGE_MIME,
      ...
      const attachment = peopleUnpackMessageAttachment(raw, item.mime);
      return peopleSendMessageAttachment(req, res, attachment);
    } catch (err) {
      console.error("[People chat/attachment get]", err);
      res.status(500).end();
    }
  }
);
...
app.post("/api/servers/:id/channels", async (req, res) => {
  try {
    ...
    if (channel?.type === "text") {
      await peopleServerE2eeEnsureState(req.params.id, channel.id, "channel-created");
    }
...
app.delete("/api/servers/:id/channels/:channelId", async (req, res) => {
  try {
    ...
    await peopleServerE2eeDropChannel(req.params.id, req.params.channelId);
    await peopleBroadcastServerChannels(req.params.id);
    res.json({ ok: true });
  } catch (err) {
...
async function peopleBroadcastServerPermissionRefresh(serverId) {
  const sid = String(serverId || "");
  if (!sid) return;
  await peopleServerE2eeRequestRotationAll(sid, "permissions");
...
// === PEOPLE_SERVER_E2EE_KEYS_V1_START ===
/*
  V11A — architecture E2EE des serveurs.

  IMPORTANT : ce bloc ne chiffre PAS encore le texte des salons. Il prépare
  la couche de distribution des clés qui sera utilisée par V11B.

  Principe :
  - une clé AES-256 est créée côté appareil pour chaque salon/epoch ;
  - le serveur ne reçoit jamais cette clé en clair ;
  - elle est enveloppée séparément pour chaque appareil autorisé via ECDH
    P-256 + HKDF-SHA256 + AES-GCM ;
  - un changement de membres/permissions déclenche un nouvel epoch ;
  - les enveloppes d'anciens epochs sont conservées pour l'historique ;
  - l'API est paginée/chunkée afin de ne pas dépendre d'une limite fixe de
    membres dans un serveur.
*/
const PEOPLE_SERVER_E2EE_PROTOCOL = "people-server-sender-key-v1";
const PEOPLE_SERVER_E2EE_SUITE = "P256-HKDF-SHA256-AES256GCM";
const PEOPLE_SERVER_E2EE_LOCAL = pathAccounts.join(
  __dirname,
  "people-server-e2ee.local.json"
);
const PEOPLE_SERVER_E2EE_CLAIM_MS = 2 * 60 * 1000;
const PEOPLE_SERVER_E2EE_RECIPIENT_PAGE = 100;
const PEOPLE_SERVER_E2EE_PACKAGE_CHUNK = 100;

function peopleServerE2eeNowIso() {
  return new Date().toISOString();
}

function peopleServerE2eeCleanReason(value) {
  return String(value || "rotation")
    .trim()
    .replace(/[\r\n\t]+/g, " ")
    .slice(0, 80) || "rotation";
}

function peopleServerE2eeCleanCipher(value, min = 16, max = 512) {
  const clean = String(value || "").trim();
  if (!new RegExp(`^[A-Za-z0-9_-]{${min},${max}}$`).test(clean)) return "";
  return clean;
}

function peopleServerE2eeReadLocal() {
  try {
    if (!fsAccounts.existsSync(PEOPLE_SERVER_E2EE_LOCAL)) {
      return { channels: [], packages: [] };
    }
    const raw = JSON.parse(fsAccounts.readFileSync(PEOPLE_SERVER_E2EE_LOCAL, "utf8"));
    return {
...
function peopleServerE2eeWriteLocal(data) {
  fsAccounts.writeFileSync(
    PEOPLE_SERVER_E2EE_LOCAL,
    JSON.stringify(
      {
...
function peopleServerE2eePublicState(row) {
  if (!row) return null;
  return {
    serverId: String(row.serverId ?? row.server_id ?? ""),
    channelId: String(row.channelId ?? row.channel_id ?? ""),
    epoch: Math.max(1, Number(row.epoch || 1)),
    status: String(row.status || "pending") === "ready" ? "ready" : "pending",
    protocol: String(row.protocol || PEOPLE_SERVER_E2EE_PROTOCOL),
    suite: String(row.suite || PEOPLE_SERVER_E2EE_SUITE),
    reason: String(row.reason || "initial"),
    commitment: String(row.keyCommitment ?? row.key_commitment ?? ""),
    ...
    sender: (row.senderUserId ?? row.sender_user_id) && (row.senderDeviceId ?? row.sender_device_id)
      ? {
          userId: String(row.senderUserId ?? row.sender_user_id),
          deviceId: String(row.senderDeviceId ?? row.sender_device_id),
          publicJwk: (() => {
            const raw = row.senderPublicJwk ?? row.sender_public_jwk;
            if (!raw) return null;
            try {
              return peopleE2eePublicJwk(typeof raw === "string" ? JSON.parse(raw) : raw);
            } catch {
              return null;
            }
          })()
...
async function peopleServerE2eeGetState(serverId, channelId) {
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  if (!sid || !cid) return null;
  ...
  if (peoplePool) {
    const result = await peoplePool.query(
      "SELECT server_id, channel_id, epoch, status, protocol, suite, reason, key_commitment, " +
      "sender_user_id, sender_device_id, sender_public_jwk, requested_at, claimed_at, ready_at " +
      "FROM people_server_e2ee_channels WHERE server_id = $1 AND channel_id = $2 LIMIT 1",
      [sid, cid]
    );
    return peopleServerE2eePublicState(result.rows[0] || null);
  }

  const data = peopleServerE2eeReadLocal();
  return peopleServerE2eePublicState(
...
async function peopleServerE2eeEnsureState(serverId, channelId, reason = "initial") {
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  const channel = await peopleGetServerChannel(sid, cid, "text");
  if (!channel) return null;

  let state = await peopleServerE2eeGetState(sid, cid);
  if (state) return state;

  if (peoplePool) {
    const result = await peoplePool.query(
      "INSERT INTO people_server_e2ee_channels " +
      "(server_id, channel_id, epoch, status, protocol, suite, reason) " +
      "VALUES ($1, $2, 1, 'pending', $3, $4, $5) " +
      "ON CONFLICT (server_id, channel_id) DO NOTHING " +
      "RETURNING server_id, channel_id, epoch, status, protocol, suite, reason, key_commitment, " +
      "sender_user_id, sender_device_id, sender_public_jwk, requested_at, claimed_at, ready_at",
      [sid, cid, PEOPLE_SERVER_E2EE_PROTOCOL, PEOPLE_SERVER_E2EE_SUITE, peopleServerE2eeCleanReason(reason)]
    );
    if (result.rows[0]) return peopleServerE2eePublicState(result.rows[0]);
    return peopleServerE2eeGetState(sid, cid);
  }
  ...
  const row = {
    ...
    epoch: 1,
    status: "pending",
    protocol: PEOPLE_SERVER_E2EE_PROTOCOL,
    suite: PEOPLE_SERVER_E2EE_SUITE,
    reason: peopleServerE2eeCleanReason(reason),
    keyCommitment: "",
    senderUserId: null,
    senderDeviceId: null,
    senderPublicJwk: null,
    requestedAt: peopleServerE2eeNowIso(),
    claimedAt: null,
    readyAt: null
  };
  data.channels.push(row);
  peopleServerE2eeWriteLocal(data);
  return peopleServerE2eePublicState(row);
}

async function peopleServerE2eeMemberIds(serverId) {
  const sid = String(serverId || "");
  if (!sid) return [];
  if (peoplePool) {
    const result = await peoplePool.query(
      "SELECT user_id FROM people_server_members WHERE server_id = $1 ORDER BY user_id ASC",
      [sid]
    );
    return result.rows.map((row) => String(row.user_id));
  }
  return peopleReadLocalServers().members
    .filter((item) => String(item.serverId ?? item.server_id ?? "") === sid)
    .map((item) => String(item.userId ?? item.user_id ?? ""))
    .filter(Boolean)
    .sort();
}

async function peopleServerE2eeRecipientDevices(serverId, channelId) {
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  const members = await peopleServerE2eeMemberIds(sid);
  const output = [];

  for (const userId of members) {
    if (!(await peopleCanServerPermission(userId, sid, "VIEW_CHANNEL", cid))) continue;
    const devices = await peopleE2eeDevices(userId);
    for (const device of devices) {
      const deviceId = peopleE2eeDeviceId(device?.deviceId);
      const publicJwk = peopleE2eePublicJwk(device?.publicJwk);
      if (!deviceId || !publicJwk) continue;
      output.push({ userId: String(userId), deviceId, publicJwk });
    }
  }

  output.sort((a, b) => {
    const userCompare = String(a.userId).localeCompare(String(b.userId), "en", { numeric: true });
    return userCompare || String(a.deviceId).localeCompare(String(b.deviceId));
  });
  return output;
}

async function peopleServerE2eeRegisteredDevice(userId, deviceId) {
  const uid = String(userId || "");
  const did = peopleE2eeDeviceId(deviceId);
  if (!uid || !did) return null;
  const devices = await peopleE2eeDevices(uid);
  return devices.find((item) => String(item.deviceId) === did) || null;
}

async function peopleServerE2eeRequestRotationChannel(serverId, channelId, reason = "rotation") {
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  const channel = await peopleGetServerChannel(sid, cid, "text");
  if (!channel) return null;
  const now = Date.now();
  const current = await peopleServerE2eeEnsureState(sid, cid, reason);
  if (!current) return null;

  // Plusieurs hooks peuvent signaler le même changement. On fusionne les
  // rotations pendantes déclenchées presque simultanément.
  const requestedMs = new Date(current.requestedAt || 0).getTime();
  if (current.status === "pending" && Number.isFinite(requestedMs) && now - requestedMs < 1500) {
    return current;
  }
  ...
  if (peoplePool) {
    const result = await peoplePool.query(
      "UPDATE people_server_e2ee_channels SET " +
      "epoch = epoch + 1, status = 'pending', reason = $3, key_commitment = '', " +
      "sender_user_id = NULL, sender_device_id = NULL, sender_public_jwk = NULL, " +
      "requested_at = NOW(), claimed_at = NULL, ready_at = NULL " +
      "WHERE server_id = $1 AND channel_id = $2 " +
      "RETURNING server_id, channel_id, epoch, status, protocol, suite, reason, key_commitment, " +
      "sender_user_id, sender_device_id, sender_public_jwk, requested_at, claimed_at, ready_at",
      [sid, cid, peopleServerE2eeCleanReason(reason)]
    );
    next = peopleServerE2eePublicState(result.rows[0]);
  } else {
    const data = peopleServerE2eeReadLocal();
    const row = data.channels.find((item) =>
      String(item.serverId ?? item.server_id ?? "") === sid &&
      String(item.channelId ?? item.channel_id ?? "") === cid
    );
    if (!row) return peopleServerE2eeEnsureState(sid, cid, reason);
    row.epoch = Math.max(1, Number(row.epoch || 1)) + 1;
    row.status = "pending";
    row.reason = peopleServerE2eeCleanReason(reason);
    row.keyCommitment = "";
    row.senderUserId = null;
    row.senderDeviceId = null;
    row.senderPublicJwk = null;
    row.requestedAt = peopleServerE2eeNowIso();
    row.claimedAt = null;
    row.readyAt = null;
    peopleServerE2eeWriteLocal(data);
    next = peopleServerE2eePublicState(row);
  }

  if (next) {
    await peopleEmitServerChannelEvent(sid, cid, "server-e2ee-rotation-needed", {
      serverId: sid,
      channelId: cid,
      epoch: next.epoch,
      reason: next.reason,
...
async function peopleServerE2eeRequestRotationAll(serverId, reason = "permissions") {
  const sid = String(serverId || "");
  if (!sid) return [];
  const channels = (await peopleListServerChannels(sid)).filter((channel) => channel.type === "text");
  const output = [];
  for (const channel of channels) {
    const state = await peopleServerE2eeRequestRotationChannel(sid, channel.id, reason);
    if (state) output.push(state);
  }
  return output;
}

async function peopleServerE2eeDropChannel(serverId, channelId) {
  if (peoplePool) return; // FK ON DELETE CASCADE fait le travail.
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  const data = peopleServerE2eeReadLocal();
  ...
  data.packages = data.packages.filter((item) => !(
    String(item.serverId ?? item.server_id ?? "") === sid &&
    String(item.channelId ?? item.channel_id ?? "") === cid
  ));
  peopleServerE2eeWriteLocal(data);
...
async function peopleServerE2eePackageForDevice(serverId, channelId, epoch, userId, deviceId) {
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  const uid = String(userId || "");
  const did = String(deviceId || "");
  const e = Number(epoch || 0);
  if (!sid || !cid || !uid || !did || !e) return null;

  if (peoplePool) {
    const result = await peoplePool.query(
      "SELECT iv, ciphertext FROM people_server_e2ee_packages " +
      "WHERE server_id = $1 AND channel_id = $2 AND epoch = $3 AND user_id = $4 AND device_id = $5 LIMIT 1",
      [sid, cid, e, uid, did]
    );
    const row = result.rows[0];
    return row ? { iv: String(row.iv), ct: String(row.ciphertext) } : null;
  }

  const row = peopleServerE2eeReadLocal().packages.find((item) =>
    String(item.serverId ?? item.server_id ?? "") === sid &&
    String(item.channelId ?? item.channel_id ?? "") === cid &&
    Number(item.epoch || 0) === e &&
    String(item.userId ?? item.user_id ?? "") === uid &&
    String(item.deviceId ?? item.device_id ?? "") === did
  );
  return row ? { iv: String(row.iv), ct: String(row.ct ?? row.ciphertext ?? "") } : null;
}

async function peopleServerE2eeClaim(serverId, channelId, accountId, deviceId, restart = false, commitment = "") {
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  const uid = String(accountId || "");
  const did = peopleE2eeDeviceId(deviceId);
  const keyCommitment = peopleServerE2eeCleanCipher(commitment, 40, 64);
  if (!sid || !cid || !uid || !did || !keyCommitment) return { ok: false, code: "INVALID" };
  if (!(await peopleCanServerPermission(uid, sid, "VIEW_CHANNEL", cid))) {
    return { ok: false, code: "FORBIDDEN" };
  }
  const device = await peopleServerE2eeRegisteredDevice(uid, did);
  if (!device?.publicJwk) return { ok: false, code: "DEVICE_UNKNOWN" };
  const publicJwk = peopleE2eePublicJwk(device.publicJwk);
  if (!publicJwk) return { ok: false, code: "DEVICE_UNKNOWN" };

  let state = await peopleServerE2eeEnsureState(sid, cid, "initial");
  if (!state || state.status !== "pending") return { ok: false, code: "NOT_PENDING", state };

  const sameSender = state.sender && state.sender.userId === uid && state.sender.deviceId === did;
  const claimedAt = new Date(state.claimedAt || 0).getTime();
  const expired = !Number.isFinite(claimedAt) || Date.now() - claimedAt > PEOPLE_SERVER_E2EE_CLAIM_MS;
  if (state.sender && !sameSender && !expired) {
    return { ok: false, code: "CLAIMED", state };
  }
  ...
  if (peoplePool) {
    ...
    try {
      ...
      const locked = await client.query(
        "SELECT epoch, status, sender_user_id, sender_device_id, claimed_at FROM people_server_e2ee_channels " +
        "WHERE server_id = $1 AND channel_id = $2 FOR UPDATE",
        [sid, cid]
      );
      const row = locked.rows[0];
      if (!row || row.status !== "pending") {
        await client.query("ROLLBACK");
        return { ok: false, code: "NOT_PENDING", state: await peopleServerE2eeGetState(sid, cid) };
      }
      const rowSame = String(row.sender_user_id || "") === uid && String(row.sender_device_id || "") === did;
      const rowClaimed = new Date(row.claimed_at || 0).getTime();
      const rowExpired = !Number.isFinite(rowClaimed) || Date.now() - rowClaimed > PEOPLE_SERVER_E2EE_CLAIM_MS;
      if (row.sender_user_id && !rowSame && !rowExpired) {
        await client.query("ROLLBACK");
        return { ok: false, code: "CLAIMED", state: await peopleServerE2eeGetState(sid, cid) };
      }

      if (restart || !rowSame || rowExpired) {
        await client.query(
          "DELETE FROM people_server_e2ee_packages WHERE server_id = $1 AND channel_id = $2 AND epoch = $3",
          [sid, cid, Number(row.epoch)]
        );
      }
      await client.query(
        "UPDATE people_server_e2ee_channels SET sender_user_id = $3, sender_device_id = $4, " +
        "sender_public_jwk = $5, key_commitment = $6, claimed_at = NOW() WHERE server_id = $1 AND channel_id = $2",
        [sid, cid, uid, did, JSON.stringify(publicJwk), keyCommitment]
      );
      await client.query("COMMIT");
    } catch (err) {
      await client.query("ROLLBACK").catch(() => {});
    ...
    if (!row || String(row.status || "") !== "pending") {
      return { ok: false, code: "NOT_PENDING", state: peopleServerE2eePublicState(row) };
    }
    const rowSame = String(row.senderUserId || "") === uid && String(row.senderDeviceId || "") === did;
    const rowClaimed = new Date(row.claimedAt || 0).getTime();
    const rowExpired = !Number.isFinite(rowClaimed) || Date.now() - rowClaimed > PEOPLE_SERVER_E2EE_CLAIM_MS;
    if (row.senderUserId && !rowSame && !rowExpired) {
      return { ok: false, code: "CLAIMED", state: peopleServerE2eePublicState(row) };
    }
    if (restart || !rowSame || rowExpired) {
      data.packages = data.packages.filter((item) => !(
        String(item.serverId ?? item.server_id ?? "") === sid &&
        String(item.channelId ?? item.channel_id ?? "") === cid &&
        Number(item.epoch || 0) === Number(row.epoch || 0)
      ));
    }
    row.senderUserId = uid;
    row.senderDeviceId = did;
    row.senderPublicJwk = publicJwk;
    row.keyCommitment = keyCommitment;
    row.claimedAt = peopleServerE2eeNowIso();
    peopleServerE2eeWriteLocal(data);
  }

  state = await peopleServerE2eeGetState(sid, cid);
  return { ok: true, state };
}

async function peopleServerE2eeStorePackageChunk(serverId, channelId, accountId, body) {
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  const uid = String(accountId || "");
  const epoch = Math.max(0, Number(body?.epoch || 0));
  const senderDeviceId = peopleE2eeDeviceId(body?.senderDeviceId);
  const packages = Array.isArray(body?.packages) ? body.packages : [];
  if (!epoch || !senderDeviceId || !packages.length || packages.length > PEOPLE_SERVER_E2EE_PACKAGE_CHUNK) {
    return { ok: false, code: "INVALID" };
  }

  const state = await peopleServerE2eeGetState(sid, cid);
  if (!state || state.status !== "pending" || state.epoch !== epoch ||
      state.sender?.userId !== uid || state.sender?.deviceId !== senderDeviceId) {
    return { ok: false, code: "CLAIM_LOST" };
  }

  const recipients = await peopleServerE2eeRecipientDevices(sid, cid);
  const allowed = new Set(recipients.map((item) => `${item.userId}\u0000${item.deviceId}`));
  const normalized = [];
  const seen = new Set();
  for (const item of packages) {
    const userId = String(item?.userId || "");
    const deviceId = peopleE2eeDeviceId(item?.deviceId);
    const iv = peopleServerE2eeCleanCipher(item?.iv, 16, 40);
    const ct = peopleServerE2eeCleanCipher(item?.ct, 32, 256);
    const key = `${userId}\u0000${deviceId}`;
    if (!userId || !deviceId || !iv || !ct || !allowed.has(key) || seen.has(key)) {
      return { ok: false, code: "RECIPIENT_INVALID" };
    }
    seen.add(key);
    normalized.push({ userId, deviceId, iv, ct });
  }
  ...
  if (peoplePool) {
    ...
    try {
      ...
      for (const item of normalized) {
        await client.query(
          "INSERT INTO people_server_e2ee_packages " +
          "(server_id, channel_id, epoch, user_id, device_id, sender_user_id, sender_device_id, iv, ciphertext) " +
          "VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) " +
          "ON CONFLICT (server_id, channel_id, epoch, user_id, device_id) DO UPDATE SET " +
          "sender_user_id = EXCLUDED.sender_user_id, sender_device_id = EXCLUDED.sender_device_id, " +
          "iv = EXCLUDED.iv, ciphertext = EXCLUDED.ciphertext, created_at = NOW()",
          [sid, cid, epoch, item.userId, item.deviceId, uid, senderDeviceId, item.iv, item.ct]
        );
      }
      await client.query("COMMIT");
    } catch (err) {
    ...
    const data = peopleServerE2eeReadLocal();
    for (const item of normalized) {
      data.packages = data.packages.filter((row) => !(
        String(row.serverId ?? row.server_id ?? "") === sid &&
        String(row.channelId ?? row.channel_id ?? "") === cid &&
        Number(row.epoch || 0) === epoch &&
        String(row.userId ?? row.user_id ?? "") === item.userId &&
        String(row.deviceId ?? row.device_id ?? "") === item.deviceId
      ));
      data.packages.push({
        serverId: sid,
        channelId: cid,
        ...
        iv: item.iv,
        ct: item.ct,
        createdAt: peopleServerE2eeNowIso()
      });
    }
    peopleServerE2eeWriteLocal(data);
  }
...
async function peopleServerE2eeFinalize(serverId, channelId, accountId, body) {
  const sid = String(serverId || "");
  const cid = String(channelId || "");
  const uid = String(accountId || "");
  const epoch = Math.max(0, Number(body?.epoch || 0));
  const senderDeviceId = peopleE2eeDeviceId(body?.senderDeviceId);
  const state = await peopleServerE2eeGetState(sid, cid);
  if (!state || state.status !== "pending" || state.epoch !== epoch || !senderDeviceId ||
      state.sender?.userId !== uid || state.sender?.deviceId !== senderDeviceId) {
    return { ok: false, code: "CLAIM_LOST" };
  }

  const expected = await peopleServerE2eeRecipientDevices(sid, cid);
  if (!expected.length) return { ok: false, code: "NO_RECIPIENTS" };
  const expectedKeys = new Set(expected.map((item) => `${item.userId}\u0000${item.deviceId}`));
  let actualKeys;

  if (peoplePool) {
    const result = await peoplePool.query(
      "SELECT user_id, device_id FROM people_server_e2ee_packages " +
      "WHERE server_id = $1 AND channel_id = $2 AND epoch = $3 AND sender_user_id = $4 AND sender_device_id = $5",
      [sid, cid, epoch, uid, senderDeviceId]
    );
    actualKeys = new Set(result.rows.map((row) => `${String(row.user_id)}\u0000${String(row.device_id)}`));
  } else {
    actualKeys = new Set(
      peopleServerE2eeReadLocal().packages
        .filter((row) =>
          String(row.serverId ?? row.server_id ?? "") === sid &&
          String(row.channelId ?? row.channel_id ?? "") === cid &&
          Number(row.epoch || 0) === epoch &&
          String(row.senderUserId ?? row.sender_user_id ?? "") === uid &&
          String(row.senderDeviceId ?? row.sender_device_id ?? "") === senderDeviceId
        )
        .map((row) => `${String(row.userId ?? row.user_id)}\u0000${String(row.deviceId ?? row.device_id)}`)
    );
  }

  if (actualKeys.size !== expectedKeys.size || [...expectedKeys].some((key) => !actualKeys.has(key))) {
    return { ok: false, code: "RECIPIENT_SET_CHANGED", expected: expectedKeys.size, stored: actualKeys.size };
  }

  if (peoplePool) {
    await peoplePool.query(
      "UPDATE people_server_e2ee_channels SET status = 'ready', ready_at = NOW() " +
      "WHERE server_id = $1 AND channel_id = $2 AND epoch = $3 AND sender_user_id = $4 AND sender_device_id = $5",
      [sid, cid, epoch, uid, senderDeviceId]
    );
  } else {
    ...
    if (!row || Number(row.epoch || 0) !== epoch) return { ok: false, code: "CLAIM_LOST" };
    row.status = "ready";
    row.readyAt = peopleServerE2eeNowIso();
    peopleServerE2eeWriteLocal(data);
  }

  const ready = await peopleServerE2eeGetState(sid, cid);
  await peopleEmitServerChannelEvent(sid, cid, "server-e2ee-key-ready", {
    serverId: sid,
    channelId: cid,
    epoch,
    protocol: PEOPLE_SERVER_E2EE_PROTOCOL
...
async function peopleInitServerE2eeV1() {
  if (peoplePool) {
    await peoplePool.query(
      "CREATE TABLE IF NOT EXISTS people_server_e2ee_channels (" +
      "server_id BIGINT NOT NULL REFERENCES people_servers(id) ON DELETE CASCADE, " +
      "channel_id BIGINT NOT NULL REFERENCES people_server_channels(id) ON DELETE CASCADE, " +
      "epoch INTEGER NOT NULL DEFAULT 1, " +
      "status VARCHAR(12) NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','ready')), " +
      "protocol VARCHAR(64) NOT NULL DEFAULT 'people-server-sender-key-v1', " +
      "suite VARCHAR(80) NOT NULL DEFAULT 'P256-HKDF-SHA256-AES256GCM', " +
      "reason VARCHAR(80) NOT NULL DEFAULT 'initial', " +
      "key_commitment VARCHAR(64) NOT NULL DEFAULT '', " +
      "sender_user_id BIGINT NULL REFERENCES people_accounts(id) ON DELETE SET NULL, " +
      "sender_device_id VARCHAR(80) NULL, " +
      "sender_public_jwk TEXT NULL, " +
    ...
    await peoplePool.query(
      "ALTER TABLE people_server_e2ee_channels ADD COLUMN IF NOT EXISTS key_commitment VARCHAR(64) NOT NULL DEFAULT ''"
    );
    await peoplePool.query(
      "CREATE TABLE IF NOT EXISTS people_server_e2ee_packages (" +
      "server_id BIGINT NOT NULL REFERENCES people_servers(id) ON DELETE CASCADE, " +
      "channel_id BIGINT NOT NULL REFERENCES people_server_channels(id) ON DELETE CASCADE, " +
      "epoch INTEGER NOT NULL, " +
      "user_id BIGINT NOT NULL REFERENCES people_accounts(id) ON DELETE CASCADE, " +
      "device_id VARCHAR(80) NOT NULL, " +
      "sender_user_id BIGINT NOT NULL REFERENCES people_accounts(id) ON DELETE CASCADE, " +
      "sender_device_id VARCHAR(80) NOT NULL, " +
      "iv VARCHAR(64) NOT NULL, " +
      "ciphertext TEXT NOT NULL, " +
      "created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), " +
      "PRIMARY KEY(server_id, channel_id, epoch, user_id, device_id)" +
      ")"
    );
    await peoplePool.query(
      "CREATE INDEX IF NOT EXISTS people_server_e2ee_packages_user_idx " +
      "ON people_server_e2ee_packages(user_id, device_id, server_id, channel_id, epoch DESC)"
    );
  }

  const servers = peoplePool
    ? (await peoplePool.query("SELECT id FROM people_servers ORDER BY id ASC")).rows.map((row) => String(row.id))
    : peopleReadLocalServers().servers.map((server) => String(server.id));

  for (const serverId of servers) {
    const channels = (await peopleListServerChannels(serverId)).filter((channel) => channel.type === "text");
    for (const channel of channels) {
      await peopleServerE2eeEnsureState(serverId, channel.id, "initial");
    }
  }

  console.log("[People] Architecture E2EE serveurs V11A prête (clés côté appareils, messages encore en clair jusqu'à V11B).");
}

function peopleServerE2eeHttpError(res, code) {
  const messages = {
    FORBIDDEN: [403, "Tu n'as pas accès à ce salon."],
    DEVICE_UNKNOWN: [400, "Cet appareil E2EE n'est pas enregistré."],
    INVALID: [400, "Requête E2EE invalide."],
    CLAIMED: [409, "Un autre appareil prépare déjà la clé de ce salon."],
    NOT_PENDING: [409, "Cet epoch E2EE n'est plus en attente."],
    CLAIM_LOST: [409, "Cet appareil ne possède plus le verrou de génération de clé."],
    RECIPIENT_INVALID: [400, "Un destinataire E2EE est invalide."],
    RECIPIENT_SET_CHANGED: [409, "La liste des appareils autorisés a changé pendant la rotation."],
    NO_RECIPIENTS: [409, "Aucun appareil E2EE autorisé n'est disponible."]
  };
  const [status, error] = messages[code] || [500, "Erreur E2EE serveur."];
  return res.status(status).json({ ok: false, code, error });
}

async function peopleServerE2eeRouteContext(req, res) {
  const session = peopleSessionForRequest(req, res);
  ...
  const channel = await peopleGetServerChannel(sid, cid, "text");
  if (!channel) {
    res.status(404).json({ ok: false, error: "Salon textuel introuvable." });
    return null;
  }
  if (!(await peopleCanServerPermission(session.id, sid, "VIEW_CHANNEL", cid))) {
    res.status(403).json({ ok: false, error: "Tu n'as pas accès à ce salon." });
    return null;
  }
  return { session, sid, cid, channel };
}

app.get("/api/servers/:id/e2ee/channels/:channelId/state", async (req, res) => {
  try {
    const context = await peopleServerE2eeRouteContext(req, res); if (!context) return;
    const deviceId = peopleE2eeDeviceId(req.query?.deviceId);
    const state = await peopleServerE2eeEnsureState(context.sid, context.cid, "initial");
    let packageForDevice = null;
    let deviceKnown = false;
    if (deviceId) {
      deviceKnown = Boolean(await peopleServerE2eeRegisteredDevice(context.session.id, deviceId));
      if (deviceKnown && state?.status === "ready") {
        packageForDevice = await peopleServerE2eePackageForDevice(
          context.sid, context.cid, state.epoch, context.session.id, deviceId
        );
      }
    }
    return res.json({
      ...
      ...state,
      deviceKnown,
      package: packageForDevice,
      claimExpiresMs: PEOPLE_SERVER_E2EE_CLAIM_MS
    });
  } catch (err) {
    console.error("[People server E2EE/state]", err);
    return res.status(500).json({ ok: false, error: "Impossible de charger l'état E2EE du salon." });
  }
});

app.post("/api/servers/:id/e2ee/channels/:channelId/claim", async (req, res) => {
  try {
    const context = await peopleServerE2eeRouteContext(req, res); if (!context) return;
    const result = await peopleServerE2eeClaim(
      context.sid, context.cid, context.session.id, req.body?.deviceId, req.body?.restart === true, req.body?.commitment
    );
    if (!result.ok) return peopleServerE2eeHttpError(res, result.code);
    return res.json({ ok: true, ...result.state });
  } catch (err) {
    console.error("[People server E2EE/claim]", err);
    return res.status(500).json({ ok: false, error: "Impossible de réserver la rotation E2EE." });
  }
});

app.get("/api/servers/:id/e2ee/channels/:channelId/recipients", async (req, res) => {
  try {
    const context = await peopleServerE2eeRouteContext(req, res); if (!context) return;
    const deviceId = peopleE2eeDeviceId(req.query?.deviceId);
    const epoch = Math.max(0, Number(req.query?.epoch || 0));
    const state = await peopleServerE2eeGetState(context.sid, context.cid);
    if (!state || state.status !== "pending" || state.epoch !== epoch ||
        state.sender?.userId !== String(context.session.id) || state.sender?.deviceId !== deviceId) {
      return peopleServerE2eeHttpError(res, "CLAIM_LOST");
    }
    const all = await peopleServerE2eeRecipientDevices(context.sid, context.cid);
    const cursor = Math.max(0, Math.floor(Number(req.query?.cursor || 0)));
    const limit = Math.max(1, Math.min(PEOPLE_SERVER_E2EE_RECIPIENT_PAGE, Math.floor(Number(req.query?.limit || PEOPLE_SERVER_E2EE_RECIPIENT_PAGE))));
    const page = all.slice(cursor, cursor + limit);
    const nextCursor = cursor + page.length < all.length ? String(cursor + page.length) : null;
    return res.json({ ok: true, epoch, recipients: page, nextCursor, total: all.length });
  } catch (err) {
    console.error("[People server E2EE/recipients]", err);
    return res.status(500).json({ ok: false, error: "Impossible de charger les appareils E2EE autorisés." });
  }
});

app.post("/api/servers/:id/e2ee/channels/:channelId/packages", async (req, res) => {
  try {
    const context = await peopleServerE2eeRouteContext(req, res); if (!context) return;
    const result = await peopleServerE2eeStorePackageChunk(
      context.sid, context.cid, context.session.id, req.body || {}
    );
    if (!result.ok) return peopleServerE2eeHttpError(res, result.code);
    return res.json(result);
  } catch (err) {
    console.error("[People server E2EE/packages]", err);
    return res.status(500).json({ ok: false, error: "Impossible d'enregistrer les enveloppes E2EE." });
  }
});

app.post("/api/servers/:id/e2ee/channels/:channelId/finalize", async (req, res) => {
  try {
    const context = await peopleServerE2eeRouteContext(req, res); if (!context) return;
    const result = await peopleServerE2eeFinalize(
      context.sid, context.cid, context.session.id, req.body || {}
    );
    if (!result.ok) return peopleServerE2eeHttpError(res, result.code);
    return res.json({ ok: true, ...result.state });
  } catch (err) {
    console.error("[People server E2EE/finalize]", err);
    return res.status(500).json({ ok: false, error: "Impossible de finaliser la clé E2EE." });
  }
});

app.post("/api/servers/:id/e2ee/channels/:channelId/request-sync", async (req, res) => {
  try {
    const context = await peopleServerE2eeRouteContext(req, res); if (!context) return;
    const deviceId = peopleE2eeDeviceId(req.body?.deviceId);
    if (!deviceId || !(await peopleServerE2eeRegisteredDevice(context.session.id, deviceId))) {
      return peopleServerE2eeHttpError(res, "DEVICE_UNKNOWN");
    }
    const state = await peopleServerE2eeEnsureState(context.sid, context.cid, "device-sync");
    if (state.status === "pending") return res.json({ ok: true, ...state });
    const existing = await peopleServerE2eePackageForDevice(
      context.sid, context.cid, state.epoch, context.session.id, deviceId
    );
    if (existing) return res.json({ ok: true, ...state, alreadyAvailable: true });
    const next = await peopleServerE2eeRequestRotationChannel(context.sid, context.cid, "device-sync");
    return res.json({ ok: true, ...next });
  } catch (err) {
    console.error("[People server E2EE/request-sync]", err);
    return res.status(500).json({ ok: false, error: "Impossible de synchroniser cet appareil E2EE." });
  }
});
// === PEOPLE_SERVER_E2EE_KEYS_V1_END ===
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!wasAlreadyMember) {
        await peopleServerE2eeRequestRotationAll(
          server.id,
          "member-joined"
        );
...
// === PEOPLE_MESSAGE_ENCRYPTION_MIGRATION_V2_START ===
async function peopleMigrateStoredMessageEncryption() {
  if (peoplePool) {
    ...
    try {
      ...
      */
      ...
      /*
        Les anciennes images étaient stockées en clair. On les migre
        par petits lots pour éviter de charger tous les BYTEA en mémoire.
        Un conteneur E2EE reste E2EE : on chiffre seulement sa couche
        de stockage, jamais son contenu côté serveur.
      */
      let lastImageId =
        "0";

      let verifiedEncryptedImage =
        false;
      ...
      while (true) {
        ...
        for (const row of imageRows.rows) {
          ...
          if (
            peopleMessageImageStorageIsEncrypted(
              row.data
            )
          ) {
            /*
              Une seule vérification complète suffit pour détecter une
              mauvaise clé sans redéchiffrer plusieurs Mo à chaque démarrage.
            */
            if (!verifiedEncryptedImage) {
              peopleDecryptMessageImageData(
                row.data
              );

              verifiedEncryptedImage =
                true;
            }
  ...
  */
  ...
  let migratedLocalImages =
    0;

  let verifiedLocalEncryptedImage =
    false;
  ...
  if (
    ...
    for (
      ...
      if (
        ...
        if (!verifiedLocalEncryptedImage) {
          peopleDecryptMessageImageData(
            stored
          );

          verifiedLocalEncryptedImage =
            true;
        }
  ...
  console.log(
    ...
    migratedLocalImages +
    " image(s) locale(s) migrée(s)."
  );
}
// === PEOPLE_MESSAGE_ENCRYPTION_MIGRATION_V2_END ===
